...
# 步骤 4: 等待服务就绪
# ==========================================
echo "⏳ 步骤 4: 等待服务启动（最多 600 秒，Qwen3-32B 大模型加载需要更长时间）..."
MAX_WAIT=600
WAIT_INTERVAL=5
ELAPSED=0
SERVICE_READY=false
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    ...
    if [ $? -eq 0 ] && [ -n "$HEALTH_RESPONSE" ]; then
        ...
        if curl -s -X POST http://localhost:8000/v1/completions \
          ...
          > /dev/null 2>&1; then
            echo "✅ 服务完全就绪（等待了 ${ELAPSED} 秒，实际请求测试通过）"
            SERVICE_READY=true
            break
        else
            echo "  ⚠️  健康检查通过但实际请求失败，继续等待..."
        fi
    fi
    
    ELAPSED=$((ELAPSED + WAIT_INTERVAL))
    if [ $((ELAPSED % 30)) -eq 0 ]; then
        echo "  ... 已等待 ${ELAPSED}/${MAX_WAIT} 秒（等待模型加载...）"
    fi
    sleep $WAIT_INTERVAL
done
...
if [ "$SERVICE_READY" != "true" ]; then
    echo ""
    echo "❌ 服务启动超时或无法处理请求（超过 ${MAX_WAIT} 秒）"
    echo ""
    echo "诊断信息："
    echo "1. 服务进程状态："
    ps -p $SERVER_PID > /dev/null 2>&1 && echo "  ✅ 进程存在" || echo "  ❌ 进程不存在"
    echo ""
    echo "2. 健康检查："
    HEALTH_RESPONSE=$(curl -s http://localhost:8000/health 2>&1)
    if [ -n "$HEALTH_RESPONSE" ]; then
        echo "  ✅ 有响应: $HEALTH_RESPONSE"
    else
        echo "  ❌ 无响应"
    fi
    echo ""
    ...
    echo ""
    echo "4. GPU 状态："
    nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader 2>/dev/null | head -4 || echo "  ⚠️  无法获取 GPU 信息"
    echo ""
    echo "建议："
    echo "  1. 查看完整日志: tail -100 $SERVER_LOG"
    echo "  2. 检查显存: nvidia-smi"
    echo "  3. 手动启动服务查看详细输出"
    exit 1
fi
